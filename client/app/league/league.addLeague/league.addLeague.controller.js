...
angular.module('abroadathletesApp')
    .controller('AddLeagueCtrl', function($scope, User, League, Upload, $location){

        $scope.logoLeague = null;
        $scope.croppedLogoLeague = null;
        $scope.createLeague = [];
        $scope.user = [];
         ...
         * Select Logo League
         */
        var selectLogoLeague = function(evt) {
            var file = evt.currentTarget.files[0];
            var reader = new FileReader();
            reader.onload = function (evt) {
                $scope.$apply(function($scope){
                    $scope.logoLeague = evt.target.result;
                });
            };
        ...
        angular.element(document.querySelector('#fileInputLogo')).on('change',selectLogoLeague);

        $scope.saveLeague = function() {


            var logoLeague = base64ToBlob($scope.croppedLogoLeague.replace('data:image/png;base64,',''), 'image/jpeg');
            ...
            Upload.upload({
                ...
                if (angular.isArray($scope.executiveLeague)) {
                    $scope.createLeague.executive = $scope.executiveLeague._id;
                }

                $scope.createLeague.logoLeague = data.photo;
                $scope.createLeague.id_user = $scope.user._id;

                var newLeague = $scope.createLeague;

               League.addLeague(newLeague).then(function(result){
